...
if __name__ == "__main__":
    ...
    Stores ROC-AUC and PR-AUC for each trained model and
    exports the results to reports/model_performance.csv.
    """
    performance_df = pd.DataFrame([
    ["Logistic Regression", results["Logistic Regression"][0], results["Logistic Regression"][1]],
    ["Random Forest", results["Random Forest"][0], results["Random Forest"][1]],
    ] + (
    [["XGBoost", results["XGBoost"][0], results["XGBoost"][1]]] if "XGBoost" in results else []
    ))

    performance_df.columns = ["Model", "ROC-AUC", "PR-AUC"]
